...
import {ImportDataService} from './importdata-ict.service'
import { AotSummaryResolver, ReturnStatement } from '@angular/compiler';
import { ToastrService } from 'ngx-toastr';
import {MatSnackBar} from '@angular/material';
import { ValidationResponse, UpdateICTReport } from './validationresponse';
...
@Component({
    selector: 'app-importdata-ict',
    templateUrl: './importdata-ict.component.html',
    styleUrls: ['./importdata-ict.component.scss'],
    encapsulation: ViewEncapsulation.None,
    providers : [ImportDataService]
  })
  export class ImportDataICTComponent {
    ...
    LoadData()    
    {
        ...
        this.dataService.loadData(this.data).subscribe(
            (data : UpdateICTReport) =>
            {

                if (data.newRecords > 0)
                {
                    this.toastrService.success(data.newRecords.toString() +  " New Records Loaded Successfully","New Records");
                }
                ...
                if (data.updatedRecords > 0)
                {
                    ...
                    if (data.updatedRecords > 1)
                    {
                        str = "Records";
                    }

                    this.toastrService.success(data.updatedRecords.toString() + " " + str  + " Updated Successfully","Records Updated");
                }

                if (data.newRecords == 0 && data.updatedRecords == 0)
                {
                    this.toastrService.success(" All records are up to date" ,"Success");
                }
            ...
            error =>
            {
                this.toastrService.error("Load Data Failed!","Error");
                this.validate = false;
                this.validating = false;    
            }
        );
